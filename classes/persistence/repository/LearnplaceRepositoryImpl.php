...
class LearnplaceRepositoryImpl implements LearnplaceRepository
{
    ...
    {
        try {
            $learnplaceEntity = \KPG\Learnplaces\persistence\entity\Learnplace::findOrFail($id);
    ...
    private function mapToEntity(Learnplace $learnplace): \KPG\Learnplaces\persistence\entity\Learnplace
    {
         ...
         */
        $activeRecord = new \KPG\Learnplaces\persistence\entity\Learnplace($learnplace->getId());
    ...
    private function storeLocationRelations(int $learnplaceId, Location $location)
    {
        try {
            /**
             * @var \KPG\Learnplaces\persistence\entity\Location $locationEntity
             */
            $locationEntity = \KPG\Learnplaces\persistence\entity\Location::findOrFail($location->getId());
    ...
    private function storeAllVisitJournals(int $learnplaceId, array $visitJournals)
    {
        try {
            ...
            foreach($visitJournals as $visitJournal) {
                 ...
                 */
                $feedbackEntity = \KPG\Learnplaces\persistence\entity\VisitJournal::findOrFail($visitJournal->getId());
    ...
    private function storeAllFeedbackRelations(int $learnplaceId, array $feedbacks)
    {
        ...
        try {
            ...
            foreach($feedbacks as $feedback) {
                 ...
                 */
                $feedbackEntity = \KPG\Learnplaces\persistence\entity\Feedback::findOrFail($feedback->getId());
                $feedbackEntity->setFkLearnplaceId($learnplaceId);
                $feedbackEntity->store();
            }
        } catch (arException $ex) {
