...
class VisitJournalRepositoryImpl implements VisitJournalRepository
{
    ...
    {
        try {
            $visitJournalEntity = \KPG\Learnplaces\persistence\entity\VisitJournal::findOrFail($id);
    ...
    {
        try {
            $visitJournal = \KPG\Learnplaces\persistence\entity\VisitJournal::findOrFail($id);
            $visitJournal->delete();
        } catch (arException $ex) {
            throw new EntityNotFoundException("VisitJournal entry with id \"$id\" not found.", $ex);
    ...
    public function findByLearnplaceId(int $id): array
    {
        return array_map(
            ...
            \KPG\Learnplaces\persistence\entity\VisitJournal::where(['fk_learnplace_id' => $id])->get()         //input
        );
    }
    ...
    public function findByObjectId(int $id): array
    {
        return array_map(
            function ($visit) { return $this->mapToDTO($visit); },                                                //mapping function
            \KPG\Learnplaces\persistence\entity\VisitJournal::innerjoinAR(
                new Learnplace(),
                'fk_learnplace_id',
                'pk_id',
                ['fk_object_id', 'pk_id']
    ...
    private function mapToDTO(\KPG\Learnplaces\persistence\entity\VisitJournal $visitJournalEntity): VisitJournal
    {
    ...
    private function mapToEntity(VisitJournal $visitJournal): \KPG\Learnplaces\persistence\entity\VisitJournal
    {

        $activeRecord = new \KPG\Learnplaces\persistence\entity\VisitJournal($visitJournal->getId());
        $activeRecord
