...
use arException;
use KPG\Learnplaces\persistence\dto\{Learnplace, PictureUploadBlock};
use KPG\Learnplaces\persistence\entity\{Block, Visibility};
...
class PictureUploadBlockRepositoryImpl implements PictureUploadBlockRepository
{
    ...
    public function findByBlockId(int $id): PictureUploadBlock
    {
        try {
             ...
             *
             * $block = Block::innerjoinAR(new \KPG\Learnplaces\persistence\entity\PictureUploadBlock(), 'pk_id', 'fk_block_id')
                ->where(['pk_id' => $id])->first();
             */

            $block = Block::findOrFail($id);
            $uploadBlock = \KPG\Learnplaces\persistence\entity\PictureUploadBlock::where(['fk_block_id' => $id])->first();
    ...
    private function mapToEntity(PictureUploadBlock $pictureUploadBlock): \KPG\Learnplaces\persistence\entity\PictureUploadBlock
    {

        /**
         * @var \KPG\Learnplaces\persistence\entity\PictureUploadBlock $activeRecord
         */
        $activeRecord = \KPG\Learnplaces\persistence\entity\PictureUploadBlock::where(['fk_block_id' => $pictureUploadBlock->getId()])->first();
        if(is_null($activeRecord)) {
            $activeRecord = new \KPG\Learnplaces\persistence\entity\PictureUploadBlock();
