...
use arException;
use KPG\Learnplaces\persistence\dto\ExternalStreamBlock;
...
class ExternalStreamBlockRepositoryImpl implements ExternalStreamBlockRepository
{
    ...
    public function findByBlockId(int $id): ExternalStreamBlock
    {
        try {
            ...
            $streamBlock = \KPG\Learnplaces\persistence\entity\ExternalStreamBlock::where(['fk_block_id' => $id])->first();
    ...
    public function findByLearnplace(Learnplace $learnplace): array
    {
         ...
         * @var Block[] $blocks
         */
        $blocks = Block::innerjoinAR(new \KPG\Learnplaces\persistence\entity\ExternalStreamBlock(), 'pk_id', 'fk_block_id')
        ...
        foreach ($blocks as $block) {
            $streamBlock = \KPG\Learnplaces\persistence\entity\ExternalStreamBlock::where(['fk_block_id' => $block->getPkId()])->first();
    ...
    private function mapToDTO(Block $block, \KPG\Learnplaces\persistence\entity\ExternalStreamBlock $streamBlockEntity): ExternalStreamBlock
    {
    ...
    private function mapToEntity(ExternalStreamBlock $streamBlock): \KPG\Learnplaces\persistence\entity\ExternalStreamBlock
    {

        /**
         * @var \KPG\Learnplaces\persistence\entity\ExternalStreamBlock $activeRecord
         */
        $activeRecord = \KPG\Learnplaces\persistence\entity\ExternalStreamBlock::where(['fk_block_id' => $streamBlock->getId()])->first();

        if(is_null($activeRecord)) {
            $activeRecord = new \KPG\Learnplaces\persistence\entity\ExternalStreamBlock();
